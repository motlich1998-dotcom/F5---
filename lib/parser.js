  ...
  function renderMirrorHtml(text, refs, resolveLabel, opts) {
    ...
    while (i < text.length) {
      ...
      if (ch === ':' && isAlphaUnderscore(next)) {
        var j = i + 1;
        while (j < text.length && isAlphaUnderscore(text[j])) j++;
        ...
        out += '<span class="' + modCls + '" '
          ...
          + 'data-args="' + escapeHtml(args) + '">'
          + escapeHtml(':' + modName + (args || '')) + '</span>';
        i = j;
        continue;
      }
      ...
      while (i < text.length) {
        ...
        var c = text[i];
        var n = (i + 1 < text.length) ? text[i + 1] : '';
        if ((c === '{' && n === '{') || (c === '}' && n === '}')) break;
        if (c === '(' || c === ')') break;
