 ...
 *  - listModifierPresets()                       — пресеты модификаторов для шаблонизатора
...
(function (global) {
  ...
  function getModifierHelp(name) {
    ...
    var map = {
      ...
      // ===== Дата / время =====
      df: {
        desc: 'Формат даты/времени по PHP-нотации (см. php.net/manual/ru/datetime.format.php). Можно вторым аргументом передать локаль (ru). Точки и двоеточия внутри формата обязательно экранируются обратной чертой: «\\.» и «\\:».',
        examples: [
      ...
      dm: {
        desc: 'Манипуляция датой: dm(формат, выражение). Меняет дату по правилам strtotime и форматирует. Точки и двоеточия внутри формата экранируются: «\\.» и «\\:».',
        examples: [
  ...
  /** Пресеты модификаторов для селектора шаблонизатора (cmdf5.ru/manual/vars). */
  function listModifierPresets() {
    return [
      {
        group: '',
        items: [{ value: '', label: '— без модификатора —' }]
      },
      {
        group: 'Дата и время',
        items: [
          { value: 'df', label: ':df — дата (стандарт)' },
          { value: 'df(d\\.m\\.Y)', label: ':df(d\\.m\\.Y) — дата' },
          { value: 'df(d\\.m\\.Y H\\:i)', label: ':df(d\\.m\\.Y H\\:i) — дата+время' },
          { value: 'df(d F,ru)', label: ':df(d F,ru) — день месяц (RU)' },
          { value: 'df(H\\:i\\:s)', label: ':df(H\\:i\\:s) — время' },
          { value: 'df(U)', label: ':df(U) — UNIX time' },
          { value: 'spell_date', label: ':spell_date — дата прописью' },
          { value: 'dm(d\\.m\\.Y,next month)', label: ':dm(...,next month) — сдвиг даты' },
          { value: 'dm(d\\.m\\.Y,last day of next month)', label: ':dm(...,last day of next month)' },
          { value: 'date_diff', label: ':date_diff — разница в днях' },
          { value: 'date_diff(, %m мес. %a дн.)', label: ':date_diff(, %m мес. %a дн.)' },
          { value: 'addworkdays(10)', label: ':addworkdays(N) — +N рабочих дней' },
          { value: 'nextworkday(d F Y, ru)', label: ':nextworkday(...) — след. рабочий день' },
          { value: 'previousworkday(d F Y, ru)', label: ':previousworkday(...) — пред. рабочий день' },
          { value: 'isworkday', label: ':isworkday — рабочий день? (1/0)' },
          { value: 'isworkday(by)', label: ':isworkday(by) — по календарю BY' }
        ]
      },
      {
        group: 'Числа и расчёты',
        items: [
          { value: 'calc', label: ':calc — посчитать выражение' },
          { value: 'format(0)', label: ':format(0) — разряды без дроби' },
          { value: 'format(2)', label: ':format(2) — 2 знака после запятой' },
          { value: 'format(2,\\,,_)', label: ':format(2,\\,,_) — разряды с _' },
          { value: 'float(1)', label: ':float(1) — дробное число' },
          { value: 'round(2)', label: ':round(2) — округление' },
          { value: 'floor', label: ':floor — округление вниз' },
          { value: 'ceil', label: ':ceil — округление вверх' },
          { value: 'currencyConvert(USD)', label: ':currencyConvert(USD) — валюта ЦБ' },
          { value: 'isEven', label: ':isEven — чётное? (1/0)' }
        ]
      },
      {
        group: 'Прописью',
        items: [
          { value: 'spell_price', label: ':spell_price — сумма прописью' },
          { value: 'spell_price(rub, short)', label: ':spell_price(rub, short)' },
          { value: 'spell_price(rub, normal)', label: ':spell_price(rub, normal)' },
          { value: 'spell_price(rub, duplication)', label: ':spell_price(rub, duplication)' },
          { value: 'spell_price(rub, clarification)', label: ':spell_price(rub, clarification)' },
          { value: 'spell_price(usd)', label: ':spell_price(usd) — доллары' },
          { value: 'spell_price(uah)', label: ':spell_price(uah) — гривны' },
          { value: 'spell_money(locale=ru, currency=rub)', label: ':spell_money(locale=ru, currency=rub)' },
          { value: 'spell_money(locale=ru, currency=uah)', label: ':spell_money(locale=ru, currency=uah)' },
          { value: 'spell_money(locale=ru, currency=kzt)', label: ':spell_money(locale=ru, currency=kzt)' },
          { value: 'spell_num', label: ':spell_num — число прописью' },
          { value: 'spell_num(2)', label: ':spell_num(2) — число в падеже' },
          { value: 'spell_ordinal', label: ':spell_ordinal — порядковое прописью' },
          { value: 'ordinal_num', label: ':ordinal_num — порядковое числительное' },
          { value: 'ordinal_num(1, f)', label: ':ordinal_num(1, f) — ж.р.' }
        ]
      },
      {
        group: 'Склонение',
        items: [
          { value: 'noun_decl(1)', label: ':noun_decl(1) — И.п.' },
          { value: 'noun_decl(2)', label: ':noun_decl(2) — Р.п.' },
          { value: 'noun_decl(3)', label: ':noun_decl(3) — Д.п.' },
          { value: 'noun_decl(4)', label: ':noun_decl(4) — В.п.' },
          { value: 'noun_decl(5)', label: ':noun_decl(5) — Т.п.' },
          { value: 'noun_decl(6)', label: ':noun_decl(6) — П.п.' },
          { value: 'noun_mdecl(1)', label: ':noun_mdecl(1) — мн. число' },
          { value: 'noun_plur(грамм)', label: ':noun_plur(слово) — окончание по числу' }
        ]
      },
      {
        group: 'Регистр',
        items: [
          { value: 'upc', label: ':upc — ВЕРХНИЙ РЕГИСТР' },
          { value: 'lwc', label: ':lwc — нижний регистр' },
      ...
      },
      {
        group: 'ФИО и гео',
        items: [
          { value: 'fio', label: ':fio — полное ФИО' },
          { value: 'fio(1)', label: ':fio(1) — имя' },
          { value: 'fio(2)', label: ':fio(2) — фамилия' },
          { value: 'fio(3)', label: ':fio(3) — отчество' },
          { value: 'fio_format(F i\\.o\\.)', label: ':fio_format(F i.o.) — Иванов И. И.' },
          { value: 'fio_format(I O F)', label: ':fio_format(I O F) — Имя Отчество Фамилия' },
          { value: 'fio:infl_name(2)', label: ':fio:infl_name(2) — склонение ФИО' },
          { value: 'infl_geo(2)', label: ':infl_geo(2) — склонение города' }
        ]
      },
      {
        group: 'Условия',
        items: [
          { value: 'if(>,9,20)', label: ':if(>,9,20) — если больше' },
          { value: 'if(>,9,20,0)', label: ':if(>,9,20,0) — иначе 0' },
          { value: 'if(=,1,да,нет)', label: ':if(=,1,да,нет) — равно' },
          { value: 'if(in,А||Б,да,нет)', label: ':if(in,...) — в списке' },
          { value: 'if(not in,А||Б,да,нет)', label: ':if(not in,...) — не в списке' },
          { value: 'if(match,текст,да,нет)', label: ':if(match,...) — содержит' },
          { value: 'if(not match,текст,да,нет)', label: ':if(not match,...) — не содержит' },
          { value: 'ifempty(—)', label: ':ifempty(—) — если пусто' },
          { value: 'ifnotempty(заполнено)', label: ':ifnotempty(...) — если заполнено' },
          { value: 'gender', label: ':gender — род (f/m)' }
        ]
      },
      {
        group: 'Род и предлоги',
        items: [
          { value: 'sp_verb(оформил)', label: ':sp_verb(...) — глагол по роду' },
          { value: 'sp_in', label: ':sp_in — предлог «в/во»' },
          { value: 'sp_with', label: ':sp_with — предлог «с/со»' },
          { value: 'sp_about', label: ':sp_about — предлог «о/об/обо»' }
        ]
      },
      {
        group: 'Строки',
        items: [
          ...
          { value: 'split(,1)', label: ':split(,N) — N-й элемент' },
          { value: 'split(,end)', label: ':split(,end) — последний элемент' },
          { value: 'split(s,1)', label: ':split(s,N) — N-е слово' },
          { value: 'slice(-1)', label: ':slice(-1) — последний символ' },
          { value: 'slice(0, 4)', label: ':slice(0, 4) — срез строки' },
          { value: 'substr(1,1)', label: ':substr(поз, длина)' },
          { value: 'replace(искать, заменить)', label: ':replace(искать, заменить)' },
          { value: 'pad_left(6, 0)', label: ':pad_left(длина, символ)' },
          { value: 'pad_right(6, 0)', label: ':pad_right(длина, символ)' },
          { value: 'nbr', label: ':nbr — переводы строк → <br>' },
          { value: 'toNumeric', label: ':toNumeric — только цифры' },
          { value: 'translit', label: ':translit — транслит' },
          { value: 'part(br)', label: ':part(br) — список с <br>' },
          { value: 'part(ul)', label: ':part(ul) — маркированный список' },
          { value: 'part(ol)', label: ':part(ol) — нумерованный список' },
          { value: 'quotemeta', label: ':quotemeta — экранировать regex' }
        ]
      },
      {
        group: 'Телефон',
        items: [
          { value: 'timezone', label: ':timezone — часовой пояс (РФ)' },
          { value: 'region', label: ':region — регион по номеру' }
        ]
      },
      {
        group: 'Прочее',
        items: [
          { value: 'qr', label: ':qr — QR-код (ссылка PNG)' }
        ]
      },
      {
        group: 'Виджет «Документы»',
        items: [
          { value: 'insertImage', label: ':insertImage — вставить картинку' },
          { value: 'insertImage(100)', label: ':insertImage(100) — ширина 100 pt' },
          { value: 'insertLink', label: ':insertLink — кликабельная ссылка' },
          { value: 'insertLink(текст ссылки)', label: ':insertLink(текст) — ссылка с текстом' },
          { value: 'insertTable', label: ':insertTable — вставить таблицу' },
          { value: 'row', label: ':row — динамическая строка товаров' }
        ]
      }
    ];
  }
  ...
  global.F5VRParser = {
    ...
    listModifierPresets: listModifierPresets,
