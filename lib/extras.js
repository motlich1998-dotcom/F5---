...
(function (global) {
  ...
  var FEATURE_CATALOG = {
    ...
    sh03: {
      ...
      desc: 'Кнопка на странице разноски: таблица для копирования и импорт отдела в Google Sheet.'
    }
  };
  ...
  function getSheetsDeptImportSettings(extras) {
    var st = extras && extras.settings && extras.settings[FEATURE_SHEETS_EXPORT];
    var legacy = extras && extras.settings && extras.settings.gub01;
    return {
      importUrl: String(st && st.importUrl || legacy && legacy.importUrl || '').trim(),
      importSecret: String(st && st.importSecret || legacy && legacy.importSecret || '').trim(),
      sheetTab: String(st && st.sheetTab || legacy && legacy.sheetTab || '').trim()
    };
  }
  ...
  global.F5VRExtras = {
    ...
    getSheetsDeptImportSettings: getSheetsDeptImportSettings,
